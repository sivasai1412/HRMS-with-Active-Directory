...
public class FetchUser{
	...
	static Logger log = Logger.getLogger(FetchUser.class);
	/**
	...
	*/
	public static void printHierarchy() throws Exception{
		...
		if(file.exists()){
			...
			log.info("\t\tHierarchy of the employees in the organization\n\t\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			...
			if(fileOne.exists()){
				...
				NodeList contractorList = documentOne.getElementsByTagName("contractor");
		...
		try{
			do{
				...
				if(input == 1){
					...
					int counts = 1;
					System.out.println("\t\t\t\t\t\t\t\t| S.No " + " ID " + "    " + "Employee Name |\n\t\t\t\t\t\t\t\t ~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
					for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
						Node node = list.item(loopCounter);
						Element element = (Element) node;
					...
					for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
						...
						if(employeeId.equalsIgnoreCase(element.getAttribute("Id"))){
							log.info("\t\tDetails of the given employee\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
				...
				try{
					...
					log.info("\t\tList of all contractors with their contractor ID\n");
					int counters = 1;
					System.out.println("\t\t\t\t\t\t\t\t| S.No " + " ID " + "    " + "Contractor Name |\n\t\t\t\t\t\t\t\t ~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
					for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
						//sample = document.getElementsByTagName("employee").item(loopCounter).getAttributes().getNamedItem("Id").getNodeValue();
						
						Node node = list.item(loopCounter);
						Element element = (Element) node;
						System.out.println("\t\t\t\t\t\t\t\t  " + counters + ". " + element.getAttribute("Id") + "     " + element.getElementsByTagName("FirstName").item(0).getTextContent());
						contractorList.add(element.getAttribute("Id"));
						counters++;
					...
					for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
						Node nodeOne = list.item(loopCounter);
						Element element = (Element) nodeOne;
						if(choice > counters-1 || choice <= 0){
							System.out.println("Please select correct option");
							fetchDetails();
						}
						else if(element.getAttribute("Id").equals(contractorList.get(choice-1))){
							contractorId = element.getAttribute("Id");
						}
					}
					...
					for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
						...
						if(contractorId.equals(element.getAttribute("Id"))){
							log.info("\t\tDetails of the given contractor\n\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
							String firstName = element.getElementsByTagName("FirstName").item(0).getTextContent();
							String lastName = element.getElementsByTagName("LastName").item(0).getTextContent();
							String designation = element.getElementsByTagName("Designation").item(0).getTextContent();
							String department = element.getElementsByTagName("Department").item(0).getTextContent();
				...
				try{
					...
					if(!file.exists()){
						...
						int counter = 0;
						log.info("\t\tList of all Managers with manager ID\n");
						...
						for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
							...
							if(element.getElementsByTagName("Designation").item(0).getTextContent().equals("Manager")){
								...
								System.out.format("\t\t\t\t\t\t\t\t  %-19s%-8s\n", element.getAttribute("Id") , element.getElementsByTagName("FirstName").item(0).getTextContent());
							}
						...
						System.out.println();
						int counts = 0;
						log.info("\t\tDetails of the employees for the manager\n\t\t\t\t\t\t\t\t~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
						for(int loopCounter = 0; loopCounter < list.getLength(); loopCounter++){
							Node node = list.item(loopCounter);	
							Element element = (Element) node;
					...
					des.contractorFileEncryption();
					if(count != list.getLength()){
						log.warn("\t\tThere is no contractor with under this manager");
					}
